...
# 交互式选择学期查询（配合 --once/--query/--check）
python -m src.main --once --semester

# 查上学期成绩（快捷方式）
python -m src.main --once --last-semester
...
# CI 模式指定学期
python -m src.main --check --semester
...
`src/main.py` 中 `ScoreMonitor` 类有四个对外方法，对应四种运行模式：

| 方法 | 触发参数 | 行为 |
|------|----------|------|
| `run()` | (无参数) | 持续循环：查成绩 → 对比 → 有变化发邮件 → 心跳检查 → sleep |
| `run_once(xnm, xqm)` | `--once` | 查一次，终端打印成绩单（含加权均分、GPA），不发邮件 |
| `run_query(xnm, xqm)` | `--query` | 查一次，**必定**发 HTML 邮件报告 |
| `run_check(xnm, xqm)` | `--check` | 查一次，与 `status/scores*.json` 对比，**有变化才发邮件** |
| `run_test()` | `--test` | 仅登录，成功后发测试邮件 |

- `--semester` 会先调用 `_choose_semester_interactive()` 弹出方向键选择界面（非 TTY 环境回退为序号输入），选完学期后传给上述方法
- `--last-semester` 通过 `_last_semester()` 推算：2-7 月→上学期为去年秋季(xqm=3)，8-1 月→上学期为同年春季(xqm=12)
